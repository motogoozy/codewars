...
function guessGifts(wishlist, presents) {
   ...
   presents.forEach(present => {
      for (let i = 0; i < wishlist.length; i++) {
         let listItem = wishlist[i];
         if (
            present.size === listItem.size &&
            present.clatters === listItem.clatters &&
            present.weight === listItem.weight &&
            !matches.includes(listItem.name)
         ) {
            matches.push(listItem.name);
         }
      }
   });
