...
function decode(str) {
  ...
  const firstLetterIdx = str.split('').findIndex(char => alphabet.includes(char));
  const num = +str.slice(0, firstLetterIdx);
  const letters = str.slice(firstLetterIdx);
  let result = [];

  letters.split('').forEach(letter => {
    for (let i = 0; i < 26; i++) {
      if ((i * num) % 26 === alphabet.indexOf(letter)) {
        result.push(alphabet[i]);
      }
    }
  });

  return result.length === letters.length ? result.join('') : 'Impossible to decode';
}
