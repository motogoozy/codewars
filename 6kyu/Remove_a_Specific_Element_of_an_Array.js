...
function selectSubarray(arr) {
	...
	arr.forEach((num, idx) => {
		...
		subArr.splice(idx, 1);
		let subSum = subArr.reduce((acc, current) => acc + current);
		let subProduct = subArr.reduce((acc, current) => acc * current);
		if (subSum !== 0 && subProduct !== 0) {
			let val = subProduct / subSum;
			map[idx] = val;
		}
	})
	...
	Object.entries(map).forEach(([index, value]) => {
		...
		if (!currentLowest || value === currentLowest) {
			currentLowest = value;
			lowest.push([index, arr[index]]);
		} else if (value < currentLowest) {
			currentLowest = value;
			lowest = [[index, arr[index]]];
		}
	})

	return lowest.length === 1 ? lowest[0] : lowest.sort((a, b) => b[0] - a[0]);
}
