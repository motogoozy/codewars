...
function stockList(stockList, categories) {
  ...
  stockList.forEach(item => {
    ...
    inventory[categoryCode] = categoryCode in inventory ? inventory[categoryCode] += +qty : +qty;
  })

  return categories.map(char => `(${char} : ${char in inventory ? inventory[char] : 0})`).join(' - ');
}
