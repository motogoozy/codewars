...
function unpackSausages(truck) {
	...
	let sausages = truck.flat(1).filter(package => {
		let productTypes = [];

		if (package.length === 6 && package.startsWith('[') && package.endsWith(']')) {
			[...package.slice(1, -1)].forEach(product => {
	...
	let filteredItems = sausages.filter((item, idx) => (idx + 1) % 5 !== 0).join('');
	let display = [...filteredItems].filter(item => {
		return item !== '[' && item !== ']';
	}).join(' ');
